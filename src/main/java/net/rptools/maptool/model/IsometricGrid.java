...
public class IsometricGrid extends Grid {
  ...
  public CellPoint convert(ZonePoint zp) {
    double isoX =
        ((zp.x - getGridOffsetX()) / getCellWidthHalf()
                + (zp.y - getGridOffsetY()) / getCellHeightHalf())
            / 2;
    double isoY =
        ((zp.y - getGridOffsetY()) / getCellHeightHalf()
                - (zp.x - getGridOffsetX()) / getCellWidthHalf())
  ...
  public ZonePoint convert(CellPoint cp) {
    double mapX = (cp.x - cp.y) * getCellWidthHalf() + getGridOffsetX();
    double mapY = (cp.x + cp.y) * getCellHeightHalf() + getGridOffsetY();
  ...
  public void draw(ZoneRenderer renderer, Graphics2D g, Rectangle bounds) {
    ...
    int offX = (int) (renderer.getViewOffsetX() % isoWidth + getGridOffsetX() * scale);
    int offY = (int) (renderer.getViewOffsetY() % gridSize + getGridOffsetY() * scale);
