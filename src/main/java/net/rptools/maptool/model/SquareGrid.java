...
public class SquareGrid extends Grid {
  ...
  public Rectangle getBounds(CellPoint cp) {
    return new Rectangle(
        cp.x * getSize() + getGridOffsetX(),
        cp.y * getSize() + getGridOffsetY(),
        getSize(),
        getSize());
  }
  ...
  public CellPoint convert(ZonePoint zp) {
    double calcX = (zp.x - getGridOffsetX()) / (float) getSize();
    double calcY = (zp.y - getGridOffsetY()) / (float) getSize();

    boolean exactCalcX = (zp.x - getGridOffsetX()) % getSize() == 0;
    boolean exactCalcY = (zp.y - getGridOffsetY()) % getSize() == 0;
  ...
  public ZonePoint convert(CellPoint cp) {
    return new ZonePoint(
        (cp.x * getSize() + getGridOffsetX()), (cp.y * getSize() + getGridOffsetY()));
  }
