...
public class MapPropertiesDialog extends JDialog {
  ...
  public void forceGridType(String gridTypeName) {
    if (GridFactory.isHexVertical(gridTypeName)) {
      getHexVerticalRadio().setSelected(true);
    } else if (GridFactory.isHexHorizontal(gridTypeName)) {
      getHexHorizontalRadio().setSelected(true);
    } else if (GridFactory.isIsometricSquare(gridTypeName)) {
      getIsometricRadio().setSelected(true);
    } else if (GridFactory.isSquare(gridTypeName)) {
  ...
  private void copyZoneToUI() {
    ...
    gridOffsetX = zone.getGrid().getGridOffsetX();
    gridOffsetY = zone.getGrid().getGridOffsetY();
  ...
  private void initIsometricRadio() {
    getIsometricRadio()
        .setSelected(GridFactory.isIsometricSquare(AppPreferences.getDefaultGridType()));
  ...
  private Grid createZoneGrid() {
    ...
    if (getHexHorizontalRadio().isSelected()) {
      grid =
          GridFactory.createGrid(
              GridType.HEX_H, AppPreferences.getFaceEdge(), AppPreferences.getFaceVertex());
    }
    if (getHexVerticalRadio().isSelected()) {
      grid =
          GridFactory.createGrid(
              GridType.HEX_V, AppPreferences.getFaceEdge(), AppPreferences.getFaceVertex());
    }
    if (getSquareRadio().isSelected()) {
      grid =
          GridFactory.createGrid(
              GridType.SQUARE, AppPreferences.getFaceEdge(), AppPreferences.getFaceVertex());
    }
    if (getIsometricRadio().isSelected()) {
      grid =
          GridFactory.createGrid(
              GridType.ISOMETRIC_SQUARE,
    ...
    grid.setGridOffset(gridOffsetX, gridOffsetY);
