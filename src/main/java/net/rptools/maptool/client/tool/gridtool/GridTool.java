...
public class GridTool extends DefaultTool {
  ...
  private final ColorWell mainColourPicker;
  private final ColorWell alternateColourPicker;
  private final JSlider zoomSlider;
  private final JTextField gridSecondDimension;
  private final JLabel gridSecondDimensionLabel;
  private final JComboBox<String> gridLineTypeSelector;
  ...
  public GridTool() {
    ...
    mainColourPicker = (ColorWell) controlPanel.getComponent("mainColourPicker");
    mainColourPicker.addActionListener(e -> copyControlPanelToGrid());
    alternateColourPicker = (ColorWell) controlPanel.getComponent("alternateColourPicker");
    alternateColourPicker.addActionListener(e -> copyControlPanelToGrid());
    gridLineTypeSelector = (JComboBox<String>) controlPanel.getComponent("GridSelectorBox");
    gridLineTypeSelector.addFocusListener(new UpdateGridListener());
  ...
  private void copyGridToControlPanel() {
    ...
    gridOffsetXTextField.setText(Integer.toString(grid.getGridOffsetX()));
    gridOffsetYTextField.setText(Integer.toString(grid.getGridOffsetY()));
    mainColourPicker.setColor(new Color(zone.getGridColor()));
    alternateColourPicker.setColor(Grid.getRenderStyle().secondColour);
    gridLineTypeSelector.setSelectedItem(Grid.getRenderStyle().lineStyle);
  ...
  private void copyControlPanelToGrid() {
    ...
    updateSecondDimension(grid, true);
    grid.setGridOffset(getInt(gridOffsetXTextField, 0), getInt(gridOffsetYTextField, 0));
    zone.setGridColor(mainColourPicker.getColor().getRGB());
    Grid.getRenderStyle().firstColour = mainColourPicker.getColor();
    Grid.getRenderStyle().secondColour = alternateColourPicker.getColor();
    // grid.getRenderStyle().twoColour =
    grid.setSize(Math.max((Integer) gridSizeSpinner.getValue(), Grid.MIN_GRID_SIZE));
    grid.setGridLineType(
        GridDrawLineStyle.valueOf(getString(gridLineTypeSelector, GridDrawLineStyle.SOLID.name())));
  }
  ...
  protected void detachFrom(ZoneRenderer renderer) {
    ...
    MapTool.serverCommand()
        .setZoneGridSize(
            zone.getId(),
            zone.getGrid().getGridOffsetX(),
            zone.getGrid().getGridOffsetY(),
  ...
  public void mousePressed(java.awt.event.MouseEvent e) {
    if (SwingUtilities.isLeftMouseButton(e)) {
      ...
      int x = zp.x - renderer.getZone().getGrid().getGridOffsetX();
      int y = zp.y - renderer.getZone().getGrid().getGridOffsetY();
  ...
  public void mouseDragged(java.awt.event.MouseEvent e) {
    if (SwingUtilities.isLeftMouseButton(e)) {
      ...
      renderer.getZone().getGrid().setGridOffset(x, y);
